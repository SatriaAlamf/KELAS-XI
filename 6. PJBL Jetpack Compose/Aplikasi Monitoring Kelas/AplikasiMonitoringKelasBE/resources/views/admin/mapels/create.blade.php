...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-xl shadow-lg p-8">
        <form action="{{ route('admin.mapels.store') }}" method="POST" class="space-y-6">
            @csrf
            <div>
                <label class="block text-sm font-semibold mb-2">Kode Mapel</label>
                <input type="text" name="kode_mapel" value="{{ old('kode_mapel') }}" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500" required>
            ...
            <div>
                ...
                <input type="text" name="nama_mapel" value="{{ old('nama_mapel') }}" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500" required>
