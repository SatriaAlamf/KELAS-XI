...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-xl shadow-lg p-8">
        <form action="{{ route('admin.tahun-ajarans.update', $tahunAjaran) }}" method="POST" class="space-y-6">
            @csrf @method('PUT')
            <div>
                <label class="block text-sm font-semibold mb-2">Tahun Ajaran</label>
                <input type="text" name="tahun" value="{{ old('tahun', $tahunAjaran->tahun) }}" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500" required>
                @error('tahun')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="flex items-center">
                    <input type="checkbox" name="flag" value="1" class="w-5 h-5 text-blue-600 rounded focus:ring-2 focus:ring-blue-500" {{ old('flag', $tahunAjaran->flag) ? 'checked' : '' }}>
